...
export const generateSavedReportPDF = (reports: Report[], reportId: string) => {
  ...
  const currentDate = new Date(savedReport.created_at).toLocaleDateString(
    'es-ES',
    {
      year: 'numeric',
      month: 'long',
  ...
  doc.text(
    `Docente: ${savedReport.professor_name || 'No disponible'}`,
    marginLeft,
    y
  );
  y += 8;
  doc.text(
    `Materia: ${savedReport.subject_name || 'No disponible'}`,
    marginLeft,
    y
  );
  y += 8;
  ...
  const summary = [
    ...
    'Comentarios:',
    ...doc.splitTextToSize(
      savedReport.comments || 'No se registraron comentarios.',
      170
    ),
    '',
    'Recomendaciones:',
    ...doc.splitTextToSize(
      savedReport.recommendations || 'No se registraron recomendaciones.',
      170
    ),
  ];
  ...
  doc.save(
    `Reporte_Historico_${savedReport.professor_name}_${currentDate.replace(/ /g, '_')}.pdf`
  );
};
...
export const generateNewReportPDF = (
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(
    `Docente: ${professor?.first_name} ${professor?.last_name}`,
    marginLeft,
    y
  );
  y += 8;
  ...
  const commentLines = doc.splitTextToSize(
    report.comments || 'No se registraron comentarios.',
    170
  );
  ...
  doc.save(
    `Reporte_Docente_${professor?.last_name}_${currentDate.replace(/ /g, '_')}.pdf`
  );
};
