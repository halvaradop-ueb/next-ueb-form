...
const initialState = (questions: Question[]) => {
  return {
    answers: questions.reduce(
      (previous, now) => ({ ...previous, [now.id]: defaultAnswer(now) }),
      {}
    ),
...
export const ProffessorForm = () => {
  ...
  const [formData, setFormData] = useState<ProfessorFormState>(
    {} as ProfessorFormState
  );
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [questionStages, setQuestionStages] = useState<
    Partial<Record<string, Question[]>>
  >({});
      ...
      <Card>
        <CardContent className="p-6">
          <div className="min-h-[300px]">
            {steps[indexStep] && steps[indexStep].component}
          </div>
          <FooterSteps
            indexStep={indexStep}
            steps={steps}
