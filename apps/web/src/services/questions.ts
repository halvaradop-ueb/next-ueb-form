    ...
    const questionsWithOptions = await Promise.all(
      questions.map(async (question: any) => {
        if (
          ...
          if (error) {
            throw new Error(
              `Error fetching options for question ${question.id}: ${error.message}`
            );
          }
...
export const addQuestion = async (
  ...
  try {
    ...
    const { data: questionId, error } = await supabase
      .from('question')
      .insert(spread)
      .select('id')
      .single();
    ...
    if (
      ...
      const questionOptions = options?.map<Omit<QuestionOptionService, 'id'>>(
        (option) => ({
...
export const cleanQuestionOptions = async (
  questionId: string
): Promise<boolean> => {
  try {
    const { error } = await supabase
...
export const getQuestionsForStudents = async (): Promise<
  [Question[], Partial<Record<string, Question[]>>]
> => {
  const questions = await getQuestions();
  const filteredQuestions = questions.filter(
    (question) => question.target_audience === 'student'
  );
  return [
    filteredQuestions,
    Object.groupBy(filteredQuestions, (question) => question.stage?.name!),
  ];
};

export const getQuestionsForProfessors = async (): Promise<
  [Question[], Partial<Record<string, Question[]>>]
> => {
  const questions = await getQuestions();
  const filteredQuestions = questions.filter(
    (question) => question.target_audience === 'professor'
  );
  return [
    filteredQuestions,
    Object.groupBy(filteredQuestions, (question) => question.stage?.name!),
